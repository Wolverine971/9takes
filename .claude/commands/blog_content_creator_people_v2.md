...
4. **TL;DR labels must be bespoke.** Do not reuse the house slot labels: `The type`, `Core type`, `The contradiction`, `The core tension`, `The childhood`, `The childhood engine`, `The pattern`, `The engine`, `The tell`, `The turn`, `The cost`, `The growth edge`. Each bold label should only make sense for this person. If the label survives a name swap, rewrite it.
...
**Deterministic pre-finalize scan:** before saving, run the local reports and use their readings rather than self-certifying freshness:

```bash
node scripts/blog-quality-report.mjs [Person-Name]
node scripts/same-type-similarity.mjs [Person-Name] --n 8
node scripts/corpus-phrase-frequency.mjs --dry
```

The contrast-pair target is **0 strong prose contrast-pair engines**. The editorial-standards skill bans negative parallelism outright, and that stricter rule wins. Do not keep "load-bearing" exceptions. If the report shows any strong contrast pairs, rewrite them as direct causality, scene, or consequence.

Read `docs/data/blog-avoid-list.md` for the subject's Enneagram type and avoid its concentrated phrases. Do not hand-copy the avoid list into the draft; use it as a warning label while writing.
...
- **One bespoke section form is required.** At least one body section must use a structure generated by this person's world: an annotated quote exchange, a scene autopsy, a timeline, a court-record style sequence, a two-date contradiction, a role-by-role pressure test, or another form that would collapse if transplanted to another subject. A normal H2 with normal paragraphs does not satisfy this.
...
**Pre-write type-challenge memo (required before drafting):** write a short working memo in your notes before Step 4. It must include:

1. strongest case for the assigned type
2. strongest case against the assigned type
3. named tiebreaker behavior or quote
4. one important behavior the type does **not** explain
5. confidence level: high / medium / low

If confidence is low, or there is no named tiebreaker, set `production_pretext.status: blocked` with `type_decision_needed` and stop before drafting in non-interactive mode. In interactive mode, ask for a human type decision.
...
**Apply the bespoke-section requirement** — at least one H2 must use a person-generated form, not the standard house slot. Name that section in the Formula Fingerprint Ledger.
...
Before saving, run `node scripts/blog-quality-report.mjs [Person-Name]` and `node scripts/same-type-similarity.mjs [Person-Name] --n 8`, then produce a **Formula Fingerprint Ledger** as an HTML comment. The ledger records deterministic report readings plus the still-human checks; it does not replace the reports.

```html
<!-- FORMULA FINGERPRINT LEDGER
Contrast-pair sentence engines: [N strong / N comparative] — from scripts/blog-quality-report.mjs
Counter-typing ladders in main body: [N] — [locations or "none"]
Same-type similarity scan: [top score + nearest same-type draft + pass/fail]
Bespoke section form: [section heading + why the form is person-specific]
Critic-pressure anchor: [named critic / skeptical reading / controversy / review + section]
Empathy turn: [criticized/mocked behavior + the motivation that reframes it + section; must be built from the person's own wound-vocabulary, not "armor/scar tissue/protection" house grammar]
...
- Any strong contrast-pair sentence engine remains in prose (`BLOG_LINT_CONTRAST_TARGET=0`).
- Any main-body counter-typing ladder compares 3+ types in sequence outside the diagnosis section or Rabbit Hole.
- The same-type similarity scan trips a near-verbatim diagnosis or counter-typing argument.
- No bespoke section form exists.
...
- [ ] **Formula Fingerprint (hard gate — Gate 6)** — Formula Fingerprint Ledger HTML comment is present and cites `blog-quality-report` + `same-type-similarity`. Strong contrast-pair sentence engines = 0, no 3+ type counter-typing ladders in the main body, no same-type similarity trip, one bespoke section form, one critic-pressure anchor, one current-tense/legacy-now anchor, and the ending passes the swap test.
...
- Deterministic quality report: `/Users/djwayne/9takes/scripts/blog-quality-report.mjs`
- Same-type similarity scan: `/Users/djwayne/9takes/scripts/same-type-similarity.mjs`
- Dynamic avoid-list: `/Users/djwayne/9takes/docs/data/blog-avoid-list.md`
