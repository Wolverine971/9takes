...
### 2026-09-24 — correction: two "Noticed" items from the Jev pass were misframed

- **"358 of 410 questions 404"** is not a bug. DJ hid them on purpose (a single bulk flag on 2026-08-14, "limit the number of questions") and none of them has a human take. Traced where links to hidden questions come from: 0 of 363 are in the sitemap, 0 appear in Search Console (90 days), 0 are linked from any blog post or source file. In three months of in-house analytics only 8 of 363 got any visit (66 visits). Before the hide those came from `/questions` and `/account`; since, about 10 "direct" one-page visits (bookmarks, history, email links). One real leak: the `/account` dashboard's question of the day, active questions and "your takes" filtered `removed` but not `flagged`, so they could send signed-in users to a 404. Fixed in `src/lib/server/accountDashboard.ts`. The 5 hidden questions that do have human takes are political ones hidden individually (voting 34 takes, Kamala 9, Iran 4, board planes 2, Charlie Kirk 2)
- **"199 people pages need hub posts"** was wrong. The count only looked at blog posts as link sources. The `/personality-analysis/categories/*` pages (linked from `/corpus-stats` and the footer) link 193 of the 195 people the Jev audit found with no blog host. The only two on no category page are Hayao Miyazaki and Dr. Phil, whose only tag is `other`, which no category maps. What those people lack is contextual blog mentions, not a hub

### 2026-09-24 — people → people pass: 169 links to 124 under-linked people pages

DJ approved linking people pages to each other ("link the pages that need links … take into account whether the page already has a ton of links"). Every sentence was read before applying; all 169 are live in the database.

- **Targets:** people pages with 2 or fewer contextual links in (blog posts + other people pages), filled to 3. 124 pages got links (42 of them had none). Pages already at 3+ got nothing.
- **Sources:** another people page naming the person in plain prose, first mention only. Pages with 10+ internal links were skipped (people pages run median 5, p90 7, max 13). At most 2 new links per source; 139 sources touched.
- **Guards:** no links in headings, blockquotes, bold spans, quoted speech, or table headers; one-word names never matched inside longer names ("J Prince", "Prince Harry"); the sentence's typing checked against the target's page.
- **Rejected on review (9, in `skipped.json`):** grief passage (Aubrey Plaza → Miles Teller, Anya Taylor-Joy), minor in a controversy (Drake → Millie Bobby Brown), insult attribution (Kris Jenner → Stephen A. Smith), negated comparison (Miyazaki → Greta Thunberg), typing conflict (Reagan page implies Jimmy Carter is 9w1; his page says 2), trivia and methodology mentions (da Vinci, Druski, a Gigi Hadid roster line). Two Prince false matches were fixed in code and replaced by real mentions (Eminem, Kamala Harris).
- **Sync:** each draft was confirmed identical to its live row first, then proven link-only (stripping internal links from the new draft gives the exact live text), synced with `personBlogParser.js <slug> --sync`, and re-checked in sync after. `lastmod` unchanged on all 139.
- **Result:** people pages with 0 contextual links in 113 → 71; with 1–2: 167 → 138; with 3–5: 109 → 180; 6+ unchanged at 61.
- **Tooling so this stays maintained:** `pnpm gen:crosslinks` now reports people pages that need links and queues people → people candidates (`link-opportunities.md` §4, `scripts/lib/peopleCrosslinks.js`); the perspective gate lets a change through without review only when it adds or removes internal links and nothing else (`scripts/lib/linkOnlyChange.js`); `/crosslink-queue` step 4b can add up to 4 people-page links per run.
- **Noticed:** Ronald Reagan's page describes a 9w1 as "more Jimmy Carter", while Carter's page types him 2 (added to T-40).
