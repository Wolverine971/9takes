...
| 4   | Re-queue or drop Brené Brown                                                      | ❌ Still in `failed` with `retryCount: 1` (unchanged since 2026-04-06)          |
| 5   | Validate rabbit hole on real published page                                       | ✅ Elon Musk pushed to DB (tracker §D confirms)                                 |
| 6   | Corpus Stats Phase 4 polish (Footer "Corpus" link, README pointer, blog cmd cite) | ❌ Not done                                                                     |
...
1. ❌ **Commit the currently-staged tree.** `git status` shows **staged but not committed**: the 7 new rabbit-hole / URL-fix / LLM-baseline docs + `docs/daily-briefs/2026-04-17_pickup-brief.md` + `scripts/personBlogParser.js` + `src/lib/server/personBlogParser.spec.ts` + admin analytics page changes (`+page.server.ts`, `+page.svelte`, both specs, `api/admin/analytics/releases/+server.ts` + spec, `docs/instagram/daily-engagement/2026-04-17_instagram-warmup.md` tweaks, `docs/research/enneagram-public-distributions.md` tweaks) + new migration `supabase/migrations/20260417_fix_release_analytics_freshness.sql`. One clean commit should land all of this before anything else moves.
...
8. ❌ **Brené Brown** still in `failed` with `retryCount: 1` since 2026-04-06 — decide: retry, manually author, or mark permanently skipped.
...
**The rabbit-hole rollout itself is effectively complete for the top-leverage pool** (Tier 4 + Class A + Tier 3 bonus). What's left is operational plumbing: commit the staged tree, capture the pre-deploy GSC baseline, deploy the URL fix, run the LLM baseline, visual-QA the retrofits, close the two Tier-3 gaps, and calendar the 2-week re-measurement. None of this is design work — it's all execution on already-written delegation docs.

**If you pick one thing tomorrow:** commit the staged tree + baseline GSC + deploy URL fix in that order. That closes the measurement-attribution window cleanly and lets the 2-week clock start.
