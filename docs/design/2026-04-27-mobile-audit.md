...
**Fix:** Either delete the gesture entirely (iOS native + Android back button cover this) or restrict to swipes that *start* within the leftmost 30px AND check that `Math.abs(deltaX) > 2 * Math.abs(deltaY)`.
