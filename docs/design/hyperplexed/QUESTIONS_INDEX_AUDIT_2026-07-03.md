...
- [shared search submit] The search component now owns custom-question navigation for both button and Enter submission. This removes a redundant route event bridge, restores Enter submission on category pages, and carries the input's current text directly so a fast Enter cannot lose the prefill. -> P13
- [live verification] Guest checks passed at 2048x1013 and 390x844 in light and dark mode: the hero has one signup action, the open-floor search has zero signup/ask actions, search expands to 720px desktop and 350px mobile, Enter stays on `/questions` without a stuck loading indicator, and all four states have zero horizontal overflow or browser-console errors. A hydrated `/questions/categories/personal-growth` follow-up also confirmed that both Enter and the visible signup button navigate to `/register` with no console errors.
- Screenshots captured:
