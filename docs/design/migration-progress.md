...
- **Admin Streetlamp polish** — 2026-05-14 sweep migrated all admin chrome from leftover Solo Leveling / hex-mix to V5 tokens (see `docs/design/admin-style-audit.md` § "V5 Streetlamp Symposium pass — 2026-05-14"). Mechanical only; still open: section kickers in JetBrains Mono, `<EmptyState>`/`<ErrorState>` atom adoption, residual `--primary-subtle`/`--primary-glow` aliases in `content-board/*`, and a real visual redesign of `asset-generators/question-print` + `zine-creator` (still on old Noticia Text aesthetic).
