...
|---|---|---|
| BUG-001 | Confirmed (type-safety) | `qrcode` options: `quality` is not valid for `image/png` (and is not in the type). Same pattern exists in `QuestionDisplay.svelte`. Use `rendererOpts.quality` with JPEG/WebP or remove. |
| BUG-002 | Confirmed (functional) | `SortComments` posts `data.id` but `QuestionPageData` only has `data.question.id`. Also see BUG-013 (missing action/route). |
| BUG-003 | Partially confirmed | Works today because `+page.svelte` aliases `ai_comments`; inconsistent naming is fragile but not currently breaking. |
| BUG-004 | Confirmed (functional) | `Links.svelte` fetches `/comments` (no `/links` endpoint exists) and uses `lastDate` from `data.comments`, so pagination is incorrect even if a links API is added. |
| BUG-005 | Confirmed (type-safety) | `FormData.append` expects string; only 2 instances now, not 4. Runtime still works via coercion. |
...
| BUG-012 | Confirmed (functional) | Removed comments render without `userHasAnswered` guard. |
...
```html
<Modal2 id={`edit-modal-${_commentComment?.id}`}>  <!-- OK -->
<Modal2 id={`flag-comment-modal-${_commentComment?.id}`}>  <!-- OK -->
```
But inside the modal:
```html
<button on:click={() => getModal(`edit-modal-${_commentComment?.id}`).close()}>
```
...
```html
class="fixed inset-0 z-[23425343]"  <!-- z-index: 23,425,343 -->
```
...
{#if _data?.removedComments?.length > 0}
  <Comments ... />  <!-- No userHasAnswered check -->
```
...
```html
<span class="text-slate-500">  <!-- May fail WCAG AA -->
```
...
```html
<button ...>
  <LeftIcon />  <!-- No visible text, relies on aria-label -->
</button>
```
