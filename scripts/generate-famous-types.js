...
async function getFamousPeople() {
	const { data: personData, error: personDataError } = await supabase
		...
		.select('person, enneagram, published, lastmod, persona_title, content_quality')
		.order('person');
...
function generateTypeScriptContent(groupedPeople) {
	...
	for (let type = 1; type <= 9; type++) {
		...
		for (let i = 0; i < people.length; i++) {
			...
			lines.push(
				`\t\t{ name: '${escapedName}', link: ${person.link}, hasImage: ${person.hasImage}, lastmod: ${lastmodValue}, personaTitle: ${personaTitleValue}, contentGrade: ${contentGradeValue} }${comma}`
			);
		}
