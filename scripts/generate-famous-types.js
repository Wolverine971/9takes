...
async function getFamousPeople() {
	const { data: personData, error: personDataError } = await supabase
		...
		.select('person, enneagram, published, lastmod, persona_title, content_quality, type')
		.order('person');

	if (personDataError) {
		console.error('❌ Error fetching famous people:', personDataError.message);
		process.exit(1);
	}
...
function normalizePeopleTypes(value) {
	let raw = [];
	...
	for (const person of people) {
		...
		if (type >= 1 && type <= 9) {
			...
			grouped[type].push({
				...
				contentGrade: person.content_quality?.overall ?? null,
				types: normalizePeopleTypes(person.type)
			});
		}
	}
...
function generateTypeScriptContent(groupedPeople) {
	...
	lines.push('export const famousTypes: { [index: number]: FamousTypePerson[] } = {');
	...
	for (let type = 1; type <= 9; type++) {
		...
		for (let i = 0; i < people.length; i++) {
			...
			const personaTitleValue = person.personaTitle
				...
				: 'null';
			const contentGradeValue = person.contentGrade ?? null;
			const typesValue = `[${person.types
				.map((type) => `'${escapeForSingleQuotes(type)}'`)
				.join(', ')}]`;
			lines.push(
				`\t\t{ name: '${escapedName}', link: ${person.link}, hasImage: ${person.hasImage}, lastmod: ${lastmodValue}, personaTitle: ${personaTitleValue}, contentGrade: ${contentGradeValue}, types: ${typesValue} }${comma}`
			);
		}
...
async function generateFamousTypes() {
	try {
		console.log('🚀 Generating famousTypes.ts...');

		const people = await getFamousPeople();
		console.log(`📊 Found ${people.length} famous people in database`);
		...
		console.log('✅ famousTypes.ts generated successfully');
