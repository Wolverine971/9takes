<script lang="ts">
	...
	const benefits = [
		{
			text: 'Approach IRL situations with the awareness of other viewpoints.',
		...
		{
			text: 'Go down the rabbit hole on any scenario/ situation you can think of.',
...
<main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			...
			<section class="hero py-12 md:py-24" in:fly={getTransition(0)}>
				<div class="hero-content flex flex-col items-center justify-between gap-8 md:flex-row">
					<div class="hero-text max-w-lg">
						...
						<h2
							class="!mb-4 !mt-0 !p-2 text-2xl font-semibold md:text-3xl"
							data-value={'9takes'}
							itemprop="name"
							on:mouseenter={() => (lightPColor = '#833BFF')}
							style="background-color: black; color: white; border-radius: 8px;     max-width: 485px;"
						>
							Comment before seeing other people's comments. <br />
							<span style={`color: ${lightPColor}`}>No more lurking</span> <br />
							<span style={`color: ${lightPColor === '#B3A6C9' ? 'white' : lightPColor};`}>+</span>
							you get
							<span
								style={`text-decoration: ${lightPColor === '#B3A6C9' ? 'none' : 'underline'}; text-decoration-color: ${lightPColor};`}
								>unbiased takes</span
							>
						</h2>
						...
						<p class="hero-description text-lg">
							We all see the world differently,<br />but some see it like you. <br />Find them here
							<br />
							on 9takes.
						</p>
					</div>
	...
	<div class="section-wrapper">
		{#if sectionsVisible[3]}
			<section class="challenges glass-area p-8" in:fly={getTransition(3)}>
				<h2 class="!m-0 !p-0 text-3xl font-bold">
					Our feelings of isolation and tension are at an all time high. Why?
				</h2>
				<h3 class="mb-4 text-2xl font-semibold">Because online conversation get stifled...</h3>
	...
	<div class="section-wrapper">
		{#if sectionsVisible[4]}
			<section class="deeper !my-12 !py-12" in:fly={getTransition(4)}>
				<h2 class="mb-4 text-3xl font-bold">Ideas that take the conversation a layer deeper</h2>
	...
	<div class="section-wrapper">
		{#if sectionsVisible[5]}
			<section class="benefits glass-area p-8" in:fly={getTransition(5)}>
				<h2 class="!m-0 !p-0 text-3xl font-bold">Your future with 9takes</h2>
				<ul class="space-y-4">
	...
	<!-- <p class="description mb-8 text-xl">
		...
		>
		<br />
		Filter comments by Personality. Understand understand yourself and others by asking questions.
	</p> -->
