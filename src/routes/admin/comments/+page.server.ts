...
export const load: PageServerLoad = async (event: any) => {
    ...
    const { data: flaggedComments, error: flaggedCommentsError } = await supabase.from('flagged_comments')
        .select(
            `*, comments 
             (*), profiles (*)`
        )
    ...
    const { data: blogComments, error: blogCommentsError } = await supabase.from('blog_comments')
        .select(
            `*`
        )
        .order('created_at', { ascending: false })
