<!-- src/routes/design-preview/harry-dry-v2/+page.svelte -->
<!-- Independent V2 copy: the current homepage remains the comparison baseline. -->
<script lang="ts">
	...
	import { faqs, practiceQuestions } from './practiceContent';
	import type { CommunityProof } from './communityProof';
	...
	onMount(() => {
		...
		async function loadProof() {
			try {
				const response = await fetch(resolve('/design-preview/harry-dry-v2/community'), {
	...
	<header class="home-nav shell">
		...
		<nav aria-label="Version comparison">
			<a href={resolve('/design-preview/harry-dry')}
				>Compare current <ArrowUpRight size={13} aria-hidden="true" /></a
			>
			<ThemeToggle />
		</nav>
	</header>
