-- supabase/migrations/20260302_admin_analytics_pages_sorted_windowed.sql
-- Created: 2026-03-02
-- Adds timestamp-window sortable page breakdown RPC for admin analytics

CREATE OR REPLACE FUNCTION public.get_page_analytics_pages_sorted_windowed(
...
AS $$
	WITH bounds AS (
		SELECT
			COALESCE(p_from_ts, NOW() - INTERVAL '30 days') AS from_ts,
			COALESCE(p_to_ts, NOW()) AS to_ts
	),
	params AS (
		SELECT
			CASE
...
GRANT EXECUTE ON FUNCTION public.get_page_analytics_pages_sorted_windowed(
	TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, INTEGER, INTEGER, TEXT, TEXT
) TO authenticated;
