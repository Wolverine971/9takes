-- supabase/migrations/20260719_fix_admin_users_page_external_id.sql
-- Keep the admin-user RPC's declared TEXT result compatible with the
-- profiles.external_id UUID column. PostgreSQL requires exact result types for
-- RETURN QUERY, so returning the UUID directly makes /admin/users fail with:
