-- supabase/migrations/20260812010400_optimize_release_performance_raw_windows.sql
-- Bound raw release analytics to the windows that can affect the report and
-- consult raw visits only when the scheduled daily rollup watermark is stale.
...
BEGIN
	...
	raw_daily_missing AS (
		...
		FROM releases r
		LEFT JOIN rollup_watermark rw ON rw.content_slug = r.slug
		INNER JOIN public.page_analytics_visits v
			...
			AND v.started_at >= GREATEST(
				r.published_at,
				(
					(
						COALESCE(
							rw.latest_metric_date,
							public.analytics_local_date(NOW() - INTERVAL '91 days')
						) + 1
	...
	raw_exact AS (
		...
		LEFT JOIN public.page_analytics_visits v
			...
			AND v.started_at < r.published_at + INTERVAL '24 hours'
			AND NOT public.is_analytics_utility_path(v.path)
		WHERE r.published_at IS NOT NULL
	...
	benchmark_24h AS (
		SELECT
			a.id,
			public.analytics_percent_rank_less(
				(RANK() OVER (ORDER BY a.views_24h) - 1)::INTEGER,
				COUNT(*) OVER ()::INTEGER
			) AS percentile,
			COUNT(*) OVER ()::INTEGER AS sample_size
		FROM agg a
	...
	benchmark_7d AS (
		SELECT
			a.id,
			public.analytics_percent_rank_less(
				(RANK() OVER (ORDER BY a.views_7d) - 1)::INTEGER,
				COUNT(*) OVER ()::INTEGER
			) AS percentile,
			COUNT(*) OVER ()::INTEGER AS sample_size
		FROM agg a
	...
	benchmark_30d AS (
		SELECT
			a.id,
			public.analytics_percent_rank_less(
				(RANK() OVER (ORDER BY a.views_30d) - 1)::INTEGER,
				COUNT(*) OVER ()::INTEGER
			) AS percentile,
			COUNT(*) OVER ()::INTEGER AS sample_size
		FROM agg a
	...
	benchmark_counts AS (
		SELECT
			...
			COALESCE((SELECT MAX(sample_size) FROM benchmark_24h), 0)::INTEGER
				AS mature_24h_count,
			COALESCE((SELECT MAX(sample_size) FROM benchmark_7d), 0)::INTEGER
				AS mature_7d_count,
			COALESCE((SELECT MAX(sample_size) FROM benchmark_30d), 0)::INTEGER
				AS mature_30d_count
		FROM rollup_bounds rb
	),
	percentiles AS (
		SELECT
			a.*,
		...
		LEFT JOIN benchmark_24h b24 ON b24.id = a.id
		LEFT JOIN benchmark_7d b7 ON b7.id = a.id
		LEFT JOIN benchmark_30d b30 ON b30.id = a.id
	),
	scored AS (
		SELECT
			p.*,
