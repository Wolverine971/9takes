-- supabase/migrations/20260827044724_optimize_trending_baseline_range.sql
-- Aggregate each baseline day inside a non-flattenable LATERAL subquery. This
-- keeps the day-specific started_at bounds on the existing B-tree index and
-- prevents the planner from turning the seven small range scans into a broad
-- visit-by-day join.
...
BEGIN
	...
	WITH params AS (
		SELECT
			COALESCE($1, NOW()) AS anchor_ts,
			LEAST(GREATEST(COALESCE($2, 7), 1), 30) AS baseline_days,
			LEAST(GREATEST(COALESCE($4, 3), 1), 200) AS min_visits,
			LEAST(GREATEST(COALESCE($5, 3), 1), 200) AS min_unique,
			LEAST(GREATEST(COALESCE($6, 20), 1), 100) AS row_limit,
			public.analytics_local_date(COALESCE($1, NOW())) AS anchor_date
	),
	bounds AS (
		SELECT
			p.*,
	...
	current_filtered AS (
		...
		WHERE v.started_at >= b.current_start_ts
			...
			AND public.analytics_scope_match($3, v.path, v.content_type)
	),
	current_grouped AS (
		SELECT
			v.path,
...
$query$
	USING p_anchor_ts, p_baseline_days, p_scope, p_min_visits, p_min_unique, p_limit;
END;
$function$;
